	...
	private class MyTimeThread extends Thread {
			...
			while (enabled) {
				...
				h.post(new Runnable() {
					...
					public void run() {
						...
						if (preferences.getBoolean("autoServiceStatus", true)) {
		...
		public void onLocationChanged(final Location location) {
			SharedPreferences pref = getApplicationContext()
					.getSharedPreferences("Settings", 0); // 0 is for mode
															// private
			...
			try {
				if (!("".equals(lastUserId))) {
					...
					queryUser.getInBackground(lastUserId,
							new GetCallback<ParseUser>() {
								...
								public void done(ParseUser usr, ParseException e) {
									...
									if (e == null) {
										...
										// &&
										// ParseUser.getCurrentUser().getObjectId().equals(usr.getObjectId()))
										// {
										usr.put("position", new ParseGeoPoint(
			...
			/*
			 * Toast.makeText(PositionService.this, ((CharSequence)
